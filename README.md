...
| 어려움 | 275 | 삼각·쌍곡선 치환, 완전제곱, 고차 삼각함수, 순환 부분적분, 유리화 치환, $\int e^{x}(f+f')$ 꼴 |
| **몬스터** | 78 | $x^{4}\pm1$·$x^{4}\pm x^{2}+1$·$x^{6}+1$ 유리식, $\sqrt{\tan x}$, $x^{x}$, 반각·역수 치환, MIT Integration Bee 결승·타이브레이커 |

난이도는 원본 사이트의 easy/medium/hard 감각에 맞췄고, **몬스터**는 그보다 한 단계 위로
따로 만든 등급입니다. 한 단계를 다 돌기 전에는 같은 문제가 다시 나오지 않으므로
쉬움은 163일, 몬스터는 78일 주기로 순환합니다.
...
problems.js     생성된 문제 은행 663문항 — 직접 고치지 말 것
