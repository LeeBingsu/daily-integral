...
| 어려움 | 168 | 두세 단계를 엮는 것들 — 유리식 점화식, 고차 삼각(6차), 중근 부분분수, 쌍곡역함수 부분적분, $\int e^{x}(f+f')$ 꼴 |
| **몬스터** | 126 | $x^{4}\pm1$·$x^{6}+1$ 유리식, $\sqrt{\tan x}$, $x^{x}$, 역수 치환 $u=x\pm\frac1x$, 괴상한 분수 지수, 몫의 미분 되짚기, MIT Integration Bee 결승·타이브레이커 |

난이도는 원본 사이트의 easy/medium/hard 감각에 맞췄고, **몬스터**는 그보다 한 단계 위로
따로 만든 등급입니다. 한 단계를 다 돌기 전에는 같은 문제가 다시 나오지 않으므로
쉬움은 174일, 몬스터는 126일 주기로 순환합니다.
...
problems.js     생성된 문제 은행 835문항 — 직접 고치지 말 것
