...
| 단계 | 문항 | 다루는 것 |
|---|---:|---|
| 쉬움 | 182 | 거듭제곱 법칙, 다항식, 기본 삼각·지수·로그, 1차식 치환, 기본 쌍곡선함수 |
| 보통 | 437 | 표준 치환·부분적분, 삼각·쌍곡선 치환표, 완전제곱, 부분분수, 곱-합 공식, 순환 부분적분 |
| 어려움 | 291 | 두세 단계를 엮는 것들 — 유리식 점화식, 고차 삼각(6차), 중근 부분분수, 쌍곡역함수 부분적분, $\int e^{x}(f+f')$ 꼴 |
| **몬스터** | 211 | $x^{4}\pm1$·$x^{6}+1$ 유리식, $\sqrt{\tan x}$, $x^{x}$, 역수 치환 $u=x\pm\frac1x$, 괴상한 분수 지수, 몫의 미분 되짚기, MIT Integration Bee 결승·타이브레이커 |

난이도는 원본 사이트의 easy/medium/hard 감각에 맞췄고, **몬스터**는 그보다 한 단계 위로
따로 만든 등급입니다. 한 단계를 다 돌기 전에는 같은 문제가 다시 나오지 않으므로
쉬움은 182일, 몬스터는 211일 주기로 순환합니다.

**MIT 적분 벌(Integration Bee) 예선 2010–2023** 문제를 전부 훑어 넣었습니다.
전체 280문항 중 259문항을 옮겼고, 이미 있던 것과 겹친 36문항을 빼면 223문항이 새로 늘었습니다.
나머지 21문항은 바닥함수·중첩 절댓값·극한 문제처럼 이 사이트의 채점기가 다루지 못하는 것들입니다
(자세한 목록은 `generate.js` 의 해당 절 주석에).
...
| 정적분 | 정적분 155문항. 답은 `pi^2/6` 같은 상수식으로 쓰고, 값이 맞으면 정답 |
...
| 연습 하루 몫 | 쉬움 무제한 · 보통 10 · 어려움 7 · 몬스터 2문제. 자정에 초기화. 한 번 꺼낸 문제는 끝낼 때까지 난이도별로 남아 있어서, 난이도를 바꿨다 돌아오거나 첫 화면에 다녀와도 같은 문제가 그대로고 몫도 줄지 않는다 — 몫은 **새 문제를 꺼낼 때만** 깎인다 |
...
| 관리자 키 | 페이지 **맨 아래**에 작게. 연습 하루 몫과 아카이브 지난 날짜를 함께 해제. 키 해시만 소스에 두지만, 브라우저 콘솔로 `localStorage` 를 고치면 뚫린다 — 접근 통제가 아니라 자기 절제용 장치. **안드로이드 앱에서는 묻지 않는다** (직접 깔아서 쓰는 것이므로 처음부터 해제 상태) |
| 아카이브 | 최근 35일 문제를 다시 풀기. 스트릭에 영향 없음. **지난 날짜는 관리자 키로 잠김** (오늘 칸은 항상 열림). 잠긴 날짜를 누르면 “권한이 없습니다” 만 뜬다 — 키는 페이지 맨 아래에서만 넣는다 |
...
`asinh/acosh/atanh`, `acot`, `sin^2(x)`, `pi` 또는 `π`, `e`, `e^(2x)` 또는 `e²ˣ`.
...
problems.js     생성된 문제 은행 1121문항 — 직접 고치지 말 것
